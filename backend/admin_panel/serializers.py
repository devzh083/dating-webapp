...
from .models import UserReport, AdminAction, PremiumPlan, PremiumFeature

class PremiumPlanSerializer(serializers.ModelSerializer):
    """Serializer for Premium Plans"""
    # Make price_per_month optional so backend can calculate it if missing
    price_per_month = serializers.DecimalField(max_digits=10, decimal_places=2, required=False)
