...
const AnimatedAvatar = ({ index }: { index: number }) => {
  ...
  return (
    <div className="relative group">
      ...
      {/* Responsive Size: w-28 on mobile, w-36 on desktop */}
      <div className={cn(
        ...
        <Sparkles className="w-8 h-8 md:w-12 md:h-12 text-white/90 drop-shadow-md" />
...
const AnonymousProfileCard = ({ profile }: AnonymousProfileCardProps) => {
  ...
  return (
    // Mobile: h-full allows it to stretch. rounded-3xl for mobile.
    <div className="relative w-full h-full rounded-[2rem] md:rounded-[40px] bg-white shadow-xl shadow-gray-200/50 border border-white overflow-hidden p-6 md:p-8 select-none flex flex-col md:block">
      
      {/* Layout: Flex col on mobile, Grid on desktop */}
      <div className="flex-1 flex flex-col md:grid md:grid-cols-12 gap-4 md:gap-8 h-full">
        ...
        <div className="flex flex-col h-full md:col-span-8 min-h-0">
          
          <div className="flex-1 flex flex-col justify-center overflow-y-auto scrollbar-hide">
            {/* Bio */}
            <div className="relative pl-4 md:pl-6 mb-4 md:mb-6 shrink-0">
              <Quote className="absolute -top-1 md:-top-2 left-0 w-4 h-4 md:w-6 md:h-6 text-gray-200 transform -scale-x-100" />
              <p className="text-gray-700 text-sm md:text-lg font-medium leading-relaxed italic">
                {selfDescription}
              </p>
            </div>

            {/* Tags */}
            <div className="flex flex-wrap gap-2 pl-1 md:pl-2 shrink-0">
              {vibeTags.slice(0, 3).map((tag) => (
                <span
                  key={tag}
                  className="px-3 py-1.5 md:px-4 md:py-2 rounded-xl bg-gray-50 border border-gray-100 text-gray-600 text-[10px] md:text-xs font-bold uppercase tracking-wide"
                >
                  {tag}
                </span>
              ))}
          ...
          {/* Starter */}
          <div className="mt-auto bg-gradient-to-r from-teal-50/80 to-emerald-50/80 rounded-2xl p-4 md:p-5 border border-teal-100 flex items-center gap-3 md:gap-4 shrink-0">
