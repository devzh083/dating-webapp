...
const HomePage = ({ onLogout }: HomePageProps) => {
  ...
  const handleSubmitStory = async () => {
    ...
    try {
      ...
      // ✅ FIXED: Use the public submit endpoint instead of admin endpoint
      const res = await fetch("http://127.0.0.1:8000/api/reviews/submit/", {
        ...
        body: JSON.stringify({
          text: storyText.trim(),
          rating: 5, // Default to 5 stars for success stories
        }),
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white pt-16 md:pt-20 overflow-x-hidden">
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-12">
        ...
        {/* 8. ✅ WRITE YOUR SUCCESS STORY SECTION */}
            ...
            <div className="relative z-10">
                ...
                <div className="mb-4 p-4 bg-green-50 rounded-xl border border-green-200 flex items-start gap-3 animate-in fade-in slide-in-from-top-2 duration-300">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-green-700">
                      Our team will review your story and feature it soon. Thank you for sharing!
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="bg-white rounded-xl md:rounded-2xl shadow-sm border border-gray-200 p-2 focus-within:ring-2 focus-within:ring-teal-500/20 focus-within:border-teal-500 transition-all">
                <textarea
                    ...
                    placeholder="Tell us how you met... (e.g., 'We matched on The Dating App and our first date was magical! We've been together for 6 months now and couldn't be happier.')"
                    value={storyText}
                ...
                <div className="flex items-center justify-between px-3 py-2 border-t border-gray-100">
                  <span className={`text-xs font-medium ${
                    storyText.length > MAX_STORY_LENGTH * 0.9 
                      ? 'text-orange-500' 
                      : 'text-gray-400'
                  }`}>
                    {storyText.length}/{MAX_STORY_LENGTH} characters
                    {storyText.length < 50 && storyText.length > 0 && (
                      <span className="ml-2 text-red-500">(min. 50)</span>
                    )}
                  </span>
